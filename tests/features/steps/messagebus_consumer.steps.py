...
def given_messagebus_consumer(ctx):
    assert "messagebus" in ctx, "'messagebus' should be in the context before calling this step"
...
def assert_processed_message(ctx, index, body):
    assert "consumer" in ctx
    index = int(index)
    assert index <= len(ctx.consumer.processed), f"requested message index exceeds number of processed messages {len(ctx.consumer.processed)}"
