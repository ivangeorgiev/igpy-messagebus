"""BDD steps for message bus sqlite persistence"""
...
def given_empty_sqlite_repository(ctx):
...
def given_initialized_empty_sqlite_repository(ctx):
...
def when_message_bus_repository_is_initialized(ctx):
...
def when_message_is_peeked(ctx):
...
def when_few_messages_are_peeked(ctx, count):
    """peek a message from the message bus and place it in context as `actual`"""
    assert (
        "messagebus" in ctx
    ), "'messagebus' should be added to the test context before calling this step"
...
def when_peeked_messages_are_deleted(ctx, count):
    ...
    assert (
        "actual" in ctx
    ), "'actual' should be added to the test context before calling this step"
    count = int(count)
    assert (
        len(ctx.actual) >= count
    ), f"expected 'actual' to contain at least {count} items, but {len(ctx.actual)} found"
    for index in range(count):
        ctx.messagebus.remove(ctx.actual[index])


@then("message bus queue table '{table_name}' exists")
def assert_message_bus_table_exists(ctx, table_name: str):
...
def assert_message_bus_queue_lock_table_exists(ctx):
    """Assert that message bus table exists"""
    assert "repository" in ctx
...
def assert_number_of_messages(ctx, expected_count):
    ...
    assert (
        len(ctx.actual) == expected_count
    ), f"Expected: {expected_count}, Actual: {len(ctx.actual)}"


@then("message body for result #{index} is '{expected_body}'")
def assert_message_body(ctx, index, expected_body):
    assert (
        "actual" in ctx
    ), "'actual' should be added to the test context before calling this step"
...
def assert_number_of_rows_in_a_table(ctx, table_name: str, expected_count: int):
    assert (
        "repository" in ctx
    ), "'repository' should be added to the test context before calling this step"
    expected_count = int(expected_count)
